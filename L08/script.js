...
// if enter is pressed any of the possible actions are executed depending on the state
...
// sums up the three parts of the story where an input is necessary, because they all have the same structure of giving you two options and changing the state accordingly
function story(option1, option2, state1, state2, elseState) {
  if (input.value.toLowerCase() == option1) {
    changeState(state1);
  } else if (input.value.toLowerCase() == option2) {
    changeState(state2);
  } else {
...
// changes the state value and outputs the corresponding text from the array
